---
jupyter:
  jupytext:
    notebook_metadata_filter: all,-language_info
    split_at_heading: true
    text_representation:
      extension: .Rmd
      format_name: rmarkdown
      format_version: '1.1'
      jupytext_version: 1.2.4
  kernelspec:
    display_name: Python 3
    language: python
    name: python3
---

In the [problem for the New York City police department]({{ site.baseurl
}}/chapters/10/confidence_problem) we had a sample of fast-track-marked exams
from 2019, and we found that the median mark was 92.  We wondered what we could
say about the eventual median when we have the marks for all 13000 or so
candidates for the exam.

After a bit of development, we found, in the [reverse probability with
bars]({{ site.baseurl }}/chapters/10/bayes_bars) page, that we could use some
probability calculations to draw conclusions about the state of the world, from
some result.  In that page, we calculated the probability of the state of the
world (a box we have been given) from a result (drawing a red ball).

Now we want to draw a conclusion about the state of the world (the eventual median of all the 2019 exams) from a result (the median of the fast-marked sample of 50 2019 exams).

We will call the 50 fast-track-marked exams the *sample*.  When the 2019
marking is finished, we will have around 8000 marks.  We will call this the
*population*.  We want to draw conclusions about the *population* from the
*sample*.   In particular we want to draw conclusions about the population
median from the sample median.

Let us start with the following problem:  what is the probability that we will
observe a *sample* median of 60, given that the *population* median is 69.

Referring back to our box and ball problem, this probability is the equivalent
of the probability of getting a red ball from a given box.  Given a state of
the world (the population median) what the is the probability of the result
(the sample median).  Once we have probabilities like these, we will be able
to use the logic you have already seen to get the *reverse* probability - how
likely was any particular state of the world (population median), given the
result (the sample median).

Returning our problem: how will we calculate the probability of a sample median of 92, given a population median of 90?

As usual, this is a problem of *sampling*.  If the median of the population is
90, and we draw a sample of 50 marks, then the median of the sample will be
vary somewhat depending on the sample. That is, the *sample median* will be 90
plus or minus a bit.  As usual, we need to quantify what we mean by "a bit".


For example, remember the population of 2017 marks, that do have a median of 90.

```{python}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
# %matplotlib inline
plt.style.use('fivethirtyeight')
```

You can download the file from [nyc_police_exam_sept_2017.csv]({{ site.baseurl
}}/data/nyc_police_exam_sept_2017.csv)


```{python}
police_2017 = pd.read_csv('nyc_police_exam_sept_2017.csv')
# Round to nearest integer.
marks_2017 = police_2017['Adj. FA'].round()
```

To get the sampling distribution of the median of a sample of 50, we would have
to calculate the median for every possible sample of 50 values from the 10000
or so marks.  As usual, we make do with an *estimate* of the sampling
distribution by taking many thousands of samples.

```{python}
# Take 10000 samples, calculate their medians.
n_iters = 10000
sample_medians = np.zeros(n_iters)
for i in np.arange(n_iters):
    sample = np.random.choice(marks_2017, size=50, replace=False)
    sample_medians[i] = np.median(sample)
sample_medians[:5]
```

The sampling distribution looks like this:

```{python}
plt.hist(sample_medians)
plt.xlabel('Median mark for sample of 50')
plt.title('Estimated sampling distribution');
```

That is the estimated sampling distribution for *2017*.  We don't know what the sampling distribution for *2019* will look like, exactly.  But --- just for the moment, let's assume that the sampling distribution for the median is the same in 2019 as it was in 2017.

If that is true then how likely are we to see a *sample* median of 92, given the state of the world that the actual population median is 90.


The first thing we do is convert the sampling distribution into probabilities.
