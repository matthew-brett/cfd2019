...
<meta property="og:url" content="https://matthew-brett.github.io/cfd2019/notebooks/04/data_frame_intro.Rmd">
...
<link rel="canonical" href="https://matthew-brett.github.io/cfd2019/notebooks/04/data_frame_intro.Rmd">
...
the [gender_stats.csv](/cfd2019/data/gender_stats.csv)
...
We can make a new data frame where the rows are sorted by the values in the
`gdp_us_billion` column:

```{python}
gender_data_by_gdp = gender_data.sort_values('gdp_us_billion')
gender_data_by_gdp.head()
```

Notice that the rows are in *ascending* order of `gdp_us_billion`.  You can
imagine, that we often want *descending* order.  As usual you can explore how
...
```{python}
by_descending_gdp = gender_data.sort_values('gdp_us_billion', ascending=False)
by_descending_gdp.head()
```
