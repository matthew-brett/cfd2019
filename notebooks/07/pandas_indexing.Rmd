...
<meta property="og:url" content="https://matthew-brett.github.io/cfd2019/notebooks/07/pandas_indexing.Rmd">
...
<link rel="canonical" href="https://matthew-brett.github.io/cfd2019/notebooks/07/pandas_indexing.Rmd">
  ...
  <article class="page" itemscope itemtype="http://schema.org/CreativeWork">
    ...
    <div class="page__inner-wrap">
      ...
      <section class="page__content" itemprop="text">
        ...
        Way back in the [introduction to data frames](/cfd2019/chapters/03/data_frame_intro), we covered indexing into data frames with
...
This is the [gender_stats.csv](/cfd2019/data/gender_stats.csv) file.
...
## You can use Boolean indexing with loc and iloc
...
We can also use Booleans with `loc` indexing.  This allows us to do things like
select rows with Booleans, and columns with labels, like this:
...
In fact you can do something similar with `iloc`, but `iloc` is aggressive
about not using the *index* (row labels) of anything.   If you try and use
a Boolean Series with `iloc`, Pandas will complain, because it can see the
*index* of `is_big_spender`, and it does not want to use it to match the Series
rows to the data frame rows, so gives an error:

```{python tags=c("raises-exception")}
# You cannot use a Boolean Series to index rows, with "iloc"
# Try to get "fert_rate" column values for big health spenders.
# "fert_rate" is the third column, at offset 2.
big_spender_fert = richest_15.iloc[is_big_spender, 2]
big_spender_fert
```
...
# Get "fert_rate" column values for big health spenders.
# "fert_rate" is the third column, at offset 2.
big_spender_fert = richest_15.iloc[is_big_spender_arr, 2]
big_spender_fert
```
