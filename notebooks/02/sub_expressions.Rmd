...
In the [expressions](Expressions) page, we saw this simple expression:

```{python}
3 * 4
```

It is an expression because it is a recipe that returns a value.  We see the notebook display the value.
...
The Notebook then makes a human-readable display for the computer representation, and shows it to us.  In this case the human-readable representation is the same as the code we typed, but this isn't always so.  Consider the expression we saw above:
...
* makes a computer representation of the number 3;
* makes a computer representation of the number 4;
* adds the representation of 3 and the representation of 4 to get
  a representation of 7.

The representation of 7 is the value returned from the expression.  The
notebook turns that into something that humans can read easily - `7`.
...
* evaluate the expression `10.50` to gives the computer representation of 10.50;
* evaluate the expression `9.25` to gives the computer representation of 10.50;
* evaluate the expression 10.50 `+` 9.25 to give the computer representation of 19.75;
* evaluate the expression 19.75 `*` 0.15 to give the computer representation of 2.9625.
...
* `10.50`: an expression that returns the computer representation of the number 10.50;
* `9.25`: an expression that returns the computer representation of the number 9.25.
...
* `10.50` as above;
* `9.25` as above;
* `0.15` returns the computer representation of the number 0.15;
* `(10.50 + 9.25)` is a compound expression that is itself a sub-expression of
  the full expression above.  It returns the computer representation of the
  number 19.75.
...
* (because of the precedence rules) evaluate  9.25 `*` 1.15 to give the
  computer representation of 1.3875;
* evaluate 10.50 `*` 1.3875 to give the computer representation of 11.8875.
...
* `0` returns the computer representation of 0;
...
* `0` returns the computer representation of 0;
* `cos( )` with argument 0, from above, returns the computer representation of
  the number 1;
* `2` returns the computer representation of 2.
...
the computer representation of the number 10.50.
