...
For our question, we were happy to assume that the line passed through 0, 0 -
meaning, that when the Hemoglobin is 0, the Packed Cell Volume value is 0.
Put another way, we assumed that our line had an *intercept* value of 0.  The
intercept y value at which the line crosses the y axis, or, put another way,
...
```{python}
sos_error_c_s([2.12, 0.51]) - sos_error_c_s([2.11, 0.51])
```
...
and a starting guess for the intercept and slope.  In this case, we start with
our guess of `[2.25, 0.47]`, but any reasonable guess will do.

```{python}
from scipy.optimize import minimize
minimize(sos_error_c_s, [2.25, 0.47])
```
