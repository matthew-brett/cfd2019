...
names = np.array(["first", "second", "third", "fourth", "fifth"])
...
#<- numbers = ...
#<- products = ...
numbers = np.array([42, 4224, 424224, -250])
...
```{python}
#<- biggest_decrease = ...
biggest_decrease = np.abs(np.min(np.diff(waiting_times)))
#<-
biggest_decrease
#<-
```
...
difference_from_expected = np.abs(np.cumsum(waiting_times) - expected)
```
...
average_error = ...
#<-
error = np.abs(np.diff(waiting_times))
