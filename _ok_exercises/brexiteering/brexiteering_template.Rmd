...
data file]({{ site.baseurl }}/data/audit_of_political_engagement_14_2017.tab)
to this directory.
...
frames]({{ site.baseurl }}/chapters/04/data_frame_intro)
...
#<- good_brexit = ...
good_brexit = mini_brexit[age_not_0]
#<--
good_brexit
```
...
```{python}
#<- remainers = ...
remainers = good_brexit[is_remain]
#<--
remainers
```
...
simulated_proportions = np.zeros(10000)
for i in np.arange(10000):
    randoms = np.random.uniform(0, 1, n_total)
    leaves = randoms < 0.519
    prop = np.count_nonzero(leaves) / n_total
    simulated_proportions[i] = prop
#<-
